...
LocalClinicalAnalisis_GeneralizedLinearModelGauss_V30<-function(D,symptom){
  ...
  covaribles<-c('intercept','time','FEP','residual_etiv','Age_inclusion','sexFemale','cpz','centile','Fep time','Fep centile','tt:cpz','time:centile')
  n<-length(covaribles)
  res <- data.frame(matrix(0,ncol=length(names_Objetives),nrow=n))
  names(res)<-names_Objetives
  row.names(res)<-covaribles
  
  
  resPValues <- data.frame(matrix(0,ncol=length(names_Objetives),nrow=n))
  names(resPValues)<-names_Objetives
  row.names(resPValues)<-covaribles
  
  
  
  for(i in 1:length(names_Objetives)){
    
    formulaText<-paste0('Global_Cognitive_Functioning_average~Treatment_Time*dx+residual_etiv+Age_inclusion+sex+CPZ_equivalent+',names_Objetives[[i]],':dx+CPZ_equivalent:Treatment_Time+',names_Objetives[[i]],'*Treatment_Time+(1|Subject)',sep='')
    
    print(formulaText)
    linealModel <- lmer(formulaText,data=D)
    print(summary(linealModel))
    
    res[,i]<-(as.numeric((coef(summary(linealModel)))[1:n,4]))
    resPValues[,i]<-(as.numeric((coef(summary(linealModel)))[1:n,5]))
