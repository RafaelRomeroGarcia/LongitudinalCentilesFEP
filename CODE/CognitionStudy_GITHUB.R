...
# Introduction:
#This script performs a mixed linear regression looking for the relationship between
#the overall cognitive functioning assessment of the cohort and covariates such as age, sex or medication.
...
# 1)function regional_brainmap_representation_borders
  ...
  #  2- column that will be the dependent variable (Y)
...
LocalClinicalAnalisis_GeneralizedLinearModelGauss_V30<-function(D,symptom){
  ...
  res <- data.frame(matrix(0,ncol=length(names_Objetives),nrow=11))
  names(res)<-names_Objetives
  row.names(res)<-c('dx','Age_inclusion','Sex','residual_etiv','CPZ_equivalent','dx0:Treatment_Time' ,'dx1:Treatment_Time'   , 'dx0:zone' ,'dx1:zone','centile:Treatment_Time','CPZ_equivalent:Treatment_Time')
  
  
  resPValues <- data.frame(matrix(0,ncol=length(names_Objetives),nrow=11))
  names(resPValues)<-names_Objetives
  row.names(resPValues)<-c('dx','Age_inclusion','Sex','residual_etiv','CPZ_equivalent','dx0:Treatment_Time' ,'dx1:Treatment_Time'   , 'dx0:zone' ,'dx1:zone','centile:Treatment_Time','CPZ_equivalent:Treatment_Time')
  
  
  
  for(i in 1:length(names_Objetives)){
    
    formulaText<-paste0(symptom,'~1+dx+Age_inclusion+Sex+residual_etiv+CPZ_equivalent+dx:Treatment_Time+dx:',names_Objetives[[i]],'+Treatment_Time:',names_Objetives[[i]],'+ CPZ_equivalent:Treatment_Time+(1|Subject)',sep='')
    
    print(formulaText)
    linealModel <- lmer(formulaText,data=D)
    print(summary(linealModel))
    
    res[,i]<-(as.numeric((coef(summary(linealModel)))[2:12,4]))
    resPValues[,i]<-(as.numeric((coef(summary(linealModel)))[2:12,5]))
  ...
  print(resPValues)
  write.csv(resPValues,'Cognition_VolumenMedio_PValues.csv')
  write.csv(res,'Cognition_VolumenMedio_ZValues.csv')
  for(z in 1:ncol(res)){
...
DataBaseGlobal<-'path /to/centiles file'
...
# Z-score of data and chage of types.
...
# Variable to study.(column of dataframe)
var_name<-'Global_Cognitive_Functioning_average'

#Linear interpolation
clinicalDataSetDataInterpolationResults<-LocalClinicalAnalisis_GeneralizedLinearModelGauss_V30(centils,var_name)
...
#save all images for all covariables
...
ggsave(paste0('path_to_images/CognitionTValues_',varNameCol,'_.svg',sep=''),dpi=400,height=5,width=8,device="svg")
