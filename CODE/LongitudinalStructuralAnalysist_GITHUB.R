...
#the centiles of the cohort and covariates such as age, sex or medication.
...
LocalClinicalAnalisis_GeneralizedLinearModelGauss_V30<-function(D){
  ...
  v<-c('dx1','Age_inclusion','Sex1','residual_etiv','CPZ_equivalent','dx0:Treatment_Time' ,'dx1:Treatment_Time' ,'CPZ_equivalent:Treatment_Time' ) #
  ...
  for(i in 1:length(names_Objetives)){
    ...
    formulaText<-paste0(names_Objetives[[i]],'~1+dx+Age_inclusion+Sex+residual_etiv+CPZ_equivalent+dx:Treatment_Time+CPZ_equivalent:Treatment_Time+(1|Subject)',sep='')
    ...
    #print(summary(linealModel))
    #summary(D)
    res[,i]<-(as.numeric((coef(summary(linealModel)))[2:n,4]))
    resPValues[,i]<-(as.numeric((coef(summary(linealModel)))[2:n,5]))
...
# Perform analysis
clinicalDataSetDataInterpolationResults<-LocalClinicalAnalisis_GeneralizedLinearModelGauss_V30(centils)

# Generate and save brain maps
var_name<-'Longitudinal Estructural Analisys '

for (i in 2:9){                               #(i covariable)
...
ggsave(paste0('path_to_file/BaseCaseStructural_TValues_',varNameCol,'_.svg',sep=''),dpi=400,height=5,width=8,device="svg")
